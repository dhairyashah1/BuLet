...
// Define which pin will output our signal (using Timer output)
#define BLE_OUTPUT_PIN BIT2  // Using P1.2 (Timer0_A2 output)
#define TIMER_CC_REG TA0CCR2  // Using CCR2 for output
...
void BLEBS_init(void)
{
    ...
    // Init GPIO - Configure P1.2 for Timer output /////////////////////////////////////////////////////////
   P1DIR |= BLE_OUTPUT_PIN;     // Set P1.2 as output
   P1SEL0 |= BLE_OUTPUT_PIN;    // Set P1.2 to peripheral function (Timer output)
   P1SEL1 &= ~BLE_OUTPUT_PIN;   // Select Timer function
...
void sendBit(uint8_t bit) {
    if (bit) {
        // For '1': Higher frequency (e.g., 2.25MHz)
        TA0CCR0 = 7;  // 16MHz/8 = 2MHz (example)
    } else {
        // For '0': Lower frequency (e.g., 1.78MHz)
        TA0CCR0 = 9;  // 16MHz/10 = 1.6MHz (example)
    }
    __delay_cycles(100);  // Hold for symbol duration
}

void sendBLEPacket() {
    uint8_t i, j;
    for (i = 0; i < len_blePacket; i++) {
        uint8_t byte = ptr_blePacket[i];
        for (j = 0; j < 8; j++) {
            sendBit((byte >> (7-j)) & 0x01);  // Send MSB first
        }
    }
}
////////////////////////////////////////////////////
